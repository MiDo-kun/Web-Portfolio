...
const BlogContent = ({ _id, title, cover, content, createdAt }) => {
  ...
  return (
    <div className="flex p-3 bg-gray-800 rounded-md mobile-lg:py-2 mobile-lg:mx-1">
      <div className="w-1/3">
      ...
      <div className="flex pl-3 flex-col justify-between w-2/3">
        ...
        <div className="flex gap-3 font-medium">
          <time className="text-xs  text-gray-300 mobile-md:text-[.71rem]">{datePosted}</time>
          <span className="text-xs  text-gray-300 mobile-md:text-[.71rem]">{readingTime(content)} min. read</span>
        </div>
      </div>
    </div>
  );
